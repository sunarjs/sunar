...
<Steps>
    ...
    <Step>
        ...
        config(slash, { // [!code highlight]
            cooldown: 5_000, // 5000 milliseconds = 5 seconds [!code highlight]
        }); // [!code highlight]
...
```js title="src/signals/cooldown.js"
import { Signal, Signals, CooldownScope, execute } from 'sunar';
...
There may be situations where you need to clear cooldowns for a user, such as when handling errors or canceling operations. Sunar provides a utility function to help with this:

```js
import { clearCooldownsForInteraction } from 'sunar';

// In your error handler or wherever you need to clear cooldowns
function handleError(interaction) {
    // Clear all cooldowns related to this interaction
    const clearedCount = clearCooldownsForInteraction(interaction);
    console.log(`Cleared ${clearedCount} cooldowns`);
...
The `clearCooldownsForInteraction` function will:
- Clear all cooldowns related to the interaction's user
- Clear all cooldowns for the interaction's channel (if any)
- Clear all cooldowns for the interaction's guild (if any)
- Return the number of cooldowns that were cleared

This is particularly useful when:
- Handling errors and you want to allow the user to retry immediately
- Canceling an operation that had cooldowns applied
- Implementing an admin override for cooldowns
- Resetting state after a failed operation
